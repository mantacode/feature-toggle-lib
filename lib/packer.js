...
//    }
//  ]
const sort = exports.sort = function(obj) {
...
const construct = exports.construct = function(arr) {
...
// 64 and 95).
const getCode = exports.getCode = function(chunk) {
  // Parse a chunk of 1 to 5 digits into binary.
  let bin = parseInt(chunk.join(''), 2);
...
const getBin = exports.getBin = function(chr) {
...
exports.pack = function(config) {
  ...
  let bits = _.map(sort(config), 'val');
...
exports.unpack = function(str, config) {
  ...
  let bits = getVals(str.split('z').slice(1));
  ...
  let unsorted = construct(sorted);
  // And return an unflattened version
  let unflat = flatten.unflatten(unsorted);
