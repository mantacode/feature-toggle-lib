...
var build = exports.build = function(config, alreadyPicked, bot, parentDisabled) {
  ...
  if ((typeof config.traffic !== 'undefined' && math.random() <= getTraffic(config, bot) && !parentDisabled) || alreadyPicked || config.version) {
    userConfig.e = 1;
  } else {
    userConfig.e = 0;
    parentDisabled = true;
  ...
  if (config.exclusiveSplit) {
    ...
    if (pick) {
      userConfig[pick] = build(config.features[pick], true, bot, parentDisabled);
    }
